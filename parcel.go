...
func (s ParcelStore) Add(p Parcel) (int, error) {
	...
	res, err := s.db.Exec("INSERT INTO parcel (client, status, address, createdAt) VALUES (:parcelClient, :parcelStatus, :parcelAddress, :parcelCreatedAt)", sql.Named("client", p.Client), sql.Named("status", p.Status), sql.Named("address", p.Address), sql.Named("createdAt", p.CreatedAt))
	if err != nil {
		return 0, err
	}
